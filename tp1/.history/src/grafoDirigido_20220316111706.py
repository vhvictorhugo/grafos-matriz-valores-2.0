    ...
    def dfs_recursiva(self, vertice, visitados):
        ...
        for vizinho in self.retornaVizinhos2(vertice):
            ...
            if vizinho not in visitados:
                ...
                if(self.ehDescendente(vertice, vizinho, [],0)):
                    return False

    def retornaPais(self, v):
        pais = []
        for i in range(len(self.matriz)):
            vizinhos = self.retornaVizinhos2(i)
            print("vizinhos", i+1, self.retornaVizinhos2(i))
        ...
        for i in range(len(pais)):
            self.ehDescendente(pais.index(i+1), w, pais,flag)


    def ehDescendente(self, v, w, pais,flag):
        
        print("pais inicio:", pais)
        print("FLAG:", flag)
        if (flag == 1):
            return True
        for p in pais:
