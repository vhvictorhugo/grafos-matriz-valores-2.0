class Grafo(object):
    ...
    def ehDescendente(self, v, w, pais, flag=0):
        if(w in pais) or (flag == 1):
            flag = 1
            return True
        print("v:", v+1)
        print("w:", w+1)
        for i in range(len(self.matriz)):
            vizinhos = self.retornaVizinhos2(v)
            print("vizinhos", i+1, self.retornaVizinhos2(v))
            if (v in vizinhos) and (v not in pais):
                pais.append(i)
                print("pais:", pais)
        for p in pais:
