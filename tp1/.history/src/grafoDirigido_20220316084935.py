class Grafo(object):
    ...
    def dfs(self):
        ...
        def dfs_iterativa(vertice_fonte):
            ...
            # falta_visitar = [vertice_fonte]   # vértices restantes
            falta_visitar = self.listarVertices()
            while falta_visitar:                                # enquanto existir vertice nao visitado
                vertice = falta_visitar.pop()
                for vizinho in self.retornaVizinhos(vertice):   # percorre vizinhos do vértice v
                    if vizinho not in visitados:                # se w é não marcado
                        ...
                        • v: vertice; w: vizinho
                        """
                        if(vertice in self.retornaVizinhos(vizinho)):
                            return False


        dfs_iterativa(vertice_fonte=1)
